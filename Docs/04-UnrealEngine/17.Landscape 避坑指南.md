...
确定单个地形的规模之后，就需要确定地形表示的最小几何精度，通常情况 1m 的水平精度足以满足RPG项目的需求，如果想要增加地表细节，可以通过以下方案来完成：
...
由于 Nanite 的几何处理路径与 RVT 的采样路径不兼容，通常 Nanite 网格并不支持 RVT 的写入，但由于地形仍保留了原始高度图和权重图数据，引擎会通过‘非 Nanite 的辅助通道’提交 RVT 绘制数据：
