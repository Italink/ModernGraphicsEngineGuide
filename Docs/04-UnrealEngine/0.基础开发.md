# Unreal Engine C++ 开发指南

## 前言

Unreal Engine 是一个非常强大的引擎，它的代码由C++编写，但独立于C++标准，实现了自己的 **构建工具（UBT）** 和 **反射编译器（UHT）** ，它跟我们常见的C++语法存在一些差异，因此网友们也戏称它为 **U++**

不仅语法，Unreal Engine下的开发流程也与平常的C++开发流程大相径庭。举例来说，STL标准库就像是一个工具箱（Toolkit），We use it to develop，而Unreal Eninge则更像是一个开发平（Platform），We develop based on it。

当下U++教程和书籍并不多，其中对开发者而言作用比较大的系列文章有：
...
此外，笔者也翻阅了一些U++视频教程，比较零碎，大多是一些披着UE皮的C++基础教程，介绍的内容有限，且存在比较多的盲区，学习收益并不是很大。

> 当然如果有发现好的教程，小伙伴也可以在评论区推荐~

Unreal Engine 匮乏的文档和晦涩的代码对C++初学者而言几乎就是一道天堑，不可逾越。

> 这是因为Unreal Engine这种工业级别的项目有着许多的开发人员，它的代码体量庞大，且迭代很快，想要整理和归纳它的发展过程是一件非常困难的事情。

想要上手Unreal Engine，必须具备中大型C++项目的工程能力，可以明确的说， **它并不适合C++初学者**

因此，如果你是一个还在上学的学生，机缘巧合下发现了Unreal Engine这么一个神奇的东西，被它绚丽的画面效果所吸引，心血来潮想要学习U++开发，那么你应该思考一下，自己除了会刷题和写论文，是否还具备以下技能：

- 熟悉 C++ 的工程构建
- 具备中大型项目源码的阅读能力，对引擎中的各个子模块有一定认知
...
-   **SWidget** 用来划分窗口区域，用于处理自身区域内的交互和绘制事件，看到它的第一反应是查阅它有哪些可供设置的属性，哪些可以覆盖的事件虚函数，以及怎么处理它PaintEvent
...
碰上 **Niagara** ，知道GPU粒子只不过是通过Compute Pipeline在交互链上模拟粒子运动，利用原子操作和间接渲染去做粒子回收，最终将粒子数据作为实例化数据绑定到粒子渲染器的参数上，从而渲染出粒子特效，而Niagara中的Module，也只是在组织Compute Shader的代码和流水线资源，因此对Niagara的性能和流程非常敏感。

看到 **Wolrd** ， **Actor** ， **Component** ， **Controller** ...，才明白Gameplay架构原来可以使用这么多结构去划分代码的职责，比我这半吊子水平做的好太多了~

诸如此类，可以说，当我放弃跟随一些文档和教程随波逐流之后，反倒是那些在通用框架上建立起来的知识体系，让我在回头审视 Unreal Engine 时，有了 ”不同寻常“ 的思考维度 —— 一个引擎里面应该有什么东西，哦，Unreal Engine里面也有，且做的很好。
