...
在计算机上，窗口是图形的载体，在GUI章节中，我们简单了解了窗口的概念，下面我将开始使用Qt封装好的窗口搭建基本的图形渲染结构：

新建一个`CMakeLists.txt`和`main.cpp`

在`CMakeLists.txt`中创建一个`Target`并链接**Qt::GuiPrivate**：
...
> 如果想让Vulkan能正常工作，需要下载 [Vulkan SDK](https://www.lunarg.com/vulkan-sdk/)
...
> 需要注意的是，对于一个窗口而言，exposeEvent会多次执行，但初始化只需要进行一次，因此需要增加一些限制。
...
使用Widget能更好的节约性能，避免不必要的刷新，但在游戏和图形这种需要实时刷新的界面时，使用Window能更好的分摊性能。
...
上面的代码中出现了很多莫名其妙的结构，暂时不用去深入考虑它们的意义，这些名词只不过是给一些特定结构和流程一个能够称呼的简短昵称，当你能够熟练使用的时候，自然会明白它们的意义。
