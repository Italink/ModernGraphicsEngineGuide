...
对于初学者而言，在现代图形API中，存在很多莫名其妙的结构，初学时不用去深入考虑它们的意义，这些名词只不过是给一些特定结构和流程一个能够称呼的简短昵称，当你了解它的应用场景和熟悉它的使用流程，自然会明白它们的意义。
...
这就要求开发者在使用图形API时，知道这些限制的存在。

除了找出符合自己功能要求的物理设备，还需要在开发时为某些设备特性不支持的用户做一些逻辑上的兼容性处理。
...
> 一天，GPU 老大 收到 代理商 送过来的 小纸条 ，立马召集小弟
>
> 开始吩咐：”唔~，这第一条，把翻斗大街的小广告都撕掉，张三，你去！“
>
> ”收到！“，说完张三就立马小跑着往翻斗大街赶去。
>
> ”这第二条是，把翻斗大街都贴满开发者的小广告，李四，你来！“
>
> “得令！”，说完李四就扫了辆小单车，往翻斗大街骑去。
>
> 翌日，开发者带着一群小弟过来，质问小广告怎么都没贴上！
>
...
在大多数 GUI 框架中，也大多提供了 Window Surface 的管理，我们要做的就是翻阅文档，深入源码。
...
- 开发者将顶点数据上传到GPU中
- 每个顶点都会被 **顶点着色器（Vertex Shader）** 处理 
...
- 每个片段都会被 **片段着色器（Fragment Shader）** 处理
- 再经由 **测试与混合阶段** 处理重叠片段后
...
> 对应QRhi中的 **QRhiTexture** 和 **QRhiSampler**
>
> ``` c++
> /*创建带有一个颜色附件的RT*/
...
如果遇到困难，可以参考 [ModernGraphicsEngineGuide](https://github.com/Italink/ModernGraphicsEngineGuide/tree/main/Source/1-GraphicsAPI) 中的 **01-WindowAndWidget** 项目，可以在[此处](https://github.com/Italink/ModernGraphicsEngineGuide/issues)提问
