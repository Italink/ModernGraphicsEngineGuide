...
private:
	...
	QScopedPointer<QRhiBuffer> mLargeUniformBuffer;
	QScopedPointer<QRhiShaderResourceBindings> mLargeShaderBindings;
...
protected:
	void initRhiResource() {
		...
		mLargeUniformBuffer.reset(mRhi->newBuffer(QRhiBuffer::Dynamic, QRhiBuffer::UniformBuffer, sizeof(UniformBlock)));
		mLargeUniformBuffer->create();

		mLargeShaderBindings.reset(mRhi->newShaderResourceBindings());
		mLargeShaderBindings->setBindings({
			QRhiShaderResourceBinding::uniformBuffer(0, QRhiShaderResourceBinding::VertexStage | QRhiShaderResourceBinding::FragmentStage, mLargeUniformBuffer.get())
		});
		mLargeShaderBindings->create();
	...
	virtual void onRenderTick() override {
		...
		QMatrix4x4 largeModel;
		largeModel.scale(1.1);
		ubo.MVP = (project * view * largeModel).toGenericMatrix<4, 4>();
		ubo.color = QVector4D(1, 0, 1, 1);
		batch->updateDynamicBuffer(mLargeUniformBuffer.get(), 0, sizeof(UniformBlock), &ubo);
		...
		cmdBuffer->setGraphicsPipeline(mPipeline.get());
		cmdBuffer->setViewport(QRhiViewport(0, 0, mSwapChain->currentPixelSize().width(), mSwapChain->currentPixelSize().height()));
		cmdBuffer->setShaderResources(mLargeShaderBindings.get());
		cmdBuffer->setVertexInput(0, 1, &cubeVertexBindings);
		cmdBuffer->draw(36);
